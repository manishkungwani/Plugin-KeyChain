...
namespace Plugin.KeyChain
{
  ...
  public class KeyChainImplementation : IKeyChain
  {
		...
		static string _serviceId = "keyChainHelperService";
		...
		public KeyChainImplementation() : this(_keyStoreFileProtectionPassword)
		{
		}

		public KeyChainImplementation(string keyStoreFileProtectionPassword)
			: this(keyStoreFileProtectionPassword, _fileName, _serviceId)
		{
		}

		public KeyChainImplementation(string keyStoreFileProtectionPassword, string fileName, string serviceId)
		{
			...
			try
			{
				lock (_fileLock)
				{
					using (var s = Application.Context.OpenFileInput(_fileName))
					{
        ...
        private void Save()
        {
            lock (_fileLock)
            {
				using (var s = Application.Context.OpenFileOutput(_fileName, FileCreationMode.Private))
                {
